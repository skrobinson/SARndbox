...
class BlobProperty { // Class to accumulate additional pixel properties along with blobs
  ...
  public:
    ...
    /* Methods: */
    void addPixel(int x, int y, const PixelParam&
                  pixelValue) { // Adds a pixel to the property accumulator
    }
    void merge(const BlobProperty&
               other) { // Merges two blob property accumulators when their respecive blobs are merged
    }
};
...
class PixelProperty { // Class to check whether a pixel should be considered part of a blob
  ...
  public:
    ...
    /* Methods: */
    bool operator()(int x, int y, const Pixel& pixel)
...
std::vector<Blob<PixelParam> > findBlobs(const int size[2], const PixelParam* frame,
        const PixelPropertyParam&
        property); // Extracts all connected blobs from the given frame whose pixels have the given property
