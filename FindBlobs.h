...
class PixelProperty { // Class to check whether a pixel should be considered part of a blob
  ...
  public:
    ...
    bool operator()(unsigned int x, unsigned int y,
                    const Pixel& pixel) const { // Returns true if the given pixel satisfies the property
        return false;
    }
};

template <class PixelParam, class PixelPropertyParam>
std::vector<Blob<PixelParam> > findBlobs(const unsigned int size[2], const PixelParam* frame,
