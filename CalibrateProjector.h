...
class CalibrateProjector: public Vrui::Application {
  ...
  private:
    ...
    ONTransform
    boxTransform; // Transformation from camera space to sandbox space (x along long sandbox axis, z up)
    ...
    void meshStreamingCallback(const Kinect::MeshBuffer&
                               meshBuffer); // Callback receiving projected meshes from the 3D video projector
#endif
    void backgroundCaptureCompleteCallback(Kinect::DirectFrameSource&
                                           camera); // Callback when the 3D camera is done capturing a background image
