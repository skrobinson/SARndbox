...
class CalibrateProjector: public Vrui::Application, public GLObject {
  ...
  private:
    ...
    struct DepthCentroidBlob: public
        Images::BboxBlob<Images::Blob<DepthPixel> > { // Structure to calculate 3D centroids of blobs in depth image space
        /* Embedded classes: */
      public:
        ...
        struct Creator: public Base::Creator {
          ...
          public:
            ...
            Kinect::FrameSource::IntrinsicParameters::PTransform
            depthProjection; // Transformation from depth image space to camera space
        };

        /* Elements: */
        Kinect::FrameSource::IntrinsicParameters::PTransform::HVector
        c; // Accumulated centroid components (x, y, z) and total weight
        ...
        DepthCentroidBlob(unsigned int x, unsigned int y, const Pixel& pixel, const Creator& creator)
            ...
            const Kinect::FrameSource::IntrinsicParameters::PTransform::Matrix& m =
                creator.depthProjection.getMatrix();
            double weight = Math::sqr(Math::sqr((m(2, 0) * px + m(2, 1) * py + m(2, 2) * pz + m(2, 3)) / (m(3,
                                                0) * px + m(3, 1) * py + m(3, 2) * pz + m(3, 3))));
        ...
        OPoint getCentroid(const Kinect::FrameSource::IntrinsicParameters::PTransform& depthProjection)
        const { // Returns the blob's centroid in camera space
  ...
  private:
    ...
    Geometry::AffineCombiner<double, 3>
    tiePointCombiner; // Combiner to average multiple tie point frames
    ...
    void backgroundCaptureCompleteCallback(Kinect::DirectFrameSource&
                                           camera); // Callback when the Kinect camera is done capturing a background image
