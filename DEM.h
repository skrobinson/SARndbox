...
class DEM: public GLObject {
  ...
  public:
    ...
    const Scalar* getDemBox(void)
    const { // Returns the DEM's bounding box as lower-left x, lower-left y, upper-right x, upper-right y
        return demBox;
    }
    ...
    const PTransform& getDemTransform(void)
    const { // Returns the full transformation from camera space to vertically-scaled DEM pixel space
        return demTransform;
    }
    Scalar getVerticalScale(void)
    const { // Returns the scaling factor from camera space elevations to DEM elevations
    ...
    void uploadDemTransform(GLint location)
    const; // Uploads the DEM transformation into the GLSL 4x4 matrix at the given uniform location
};
