...
void BathymetrySaverTool::writeDEMFile() const {
    ...
    for(GLsizei column = 0; column < factory->gridSize[0]; ++column) {
        ...
        printFloat8(demFile, west + double(column)*double(
                        factory->cellSize[0])); // Easting of first elevation posting in column
