...
class WaterTable2: public GLObject {
  ...
  private:
    ...
    /* Elements: */
    static const char*
    vertexShaderSource; // Source code of vertex shader shared by all shader programs
    ...
    /* Constructors and destructors: */
  public:
    WaterTable2(GLsizei width, GLsizei height, const Plane& basePlane,
                const Point
                basePlaneCorners[4]); // Creates a water table of the given size in pixels, for the base plane quadrilateral defined by the plane equation and four corner points
    ...
    /* New methods: */
    const Transform& getBaseTransform(void)
    ...
    void updateBathymetry(const SurfaceRenderer& bathymetryRenderer,
                          GLContextData& contextData)
    const; // Renders the given surface into the bathymetry grid used for subsequent simulation steps
    GLfloat runSimulationStep(GLContextData& contextData)
    const; // Runs a water flow simulation step; returns step size taken by Runge-Kutta integration step
    ...
    const GLfloat* getWaterTextureMatrix(void)
