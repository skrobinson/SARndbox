...
class BlobForegroundSelector { // Functor class to select foreground pixels
  ...
  public:
    bool operator()(unsigned int x, unsigned int y,
                    const Kinect::FrameSource::DepthPixel& pixel) const {
...
class BlobMergeChecker { // Functor class to check whether two pixels can belong to the same blob
  ...
  public:
    ...
    /* Methods: */
    bool operator()(unsigned int x1, unsigned int y1, const Kinect::FrameSource::DepthPixel& pixel1,
                    unsigned int x2, unsigned int y2, const Kinect::FrameSource::DepthPixel& pixel2) const {
...
void CalibrateProjector::frame(void) {
    ...
    if(rawFrames.lockNewValue()) {
        ...
        const DepthPixel* framePixels = static_cast<const DepthPixel*>
                                        (rawFrames.getLockedValue().getBuffer());
        ...
        std::vector<DepthCentroidBlob> blobs = Images::extractBlobs<DepthCentroidBlob>(frameSize,
                                               framePixels, bfs, bmc, blobCreator, blobIdImage);
        ...
        for(std::vector<DepthCentroidBlob>::iterator bIt = blobs.begin(); bIt != blobs.end(); ++bIt)
            if(maxNumPixels < bIt->numPixels) {
                ...
                bool inside = true;
                inside = inside
                         && Geometry::cross(basePlane.getNormal(),
                                            basePlaneCorners[1] - basePlaneCorners[0]) * (blobCentroid - basePlaneCorners[0]) >= 0.0;
                inside = inside
                         && Geometry::cross(basePlane.getNormal(),
                                            basePlaneCorners[3] - basePlaneCorners[1]) * (blobCentroid - basePlaneCorners[1]) >= 0.0;
                inside = inside
                         && Geometry::cross(basePlane.getNormal(),
                                            basePlaneCorners[2] - basePlaneCorners[3]) * (blobCentroid - basePlaneCorners[3]) >= 0.0;
                inside = inside
                         && Geometry::cross(basePlane.getNormal(),
                                            basePlaneCorners[0] - basePlaneCorners[2]) * (blobCentroid - basePlaneCorners[2]) >= 0.0;
                double fillRatio = double(bIt->numPixels) / (double(bIt->bbMax[0] - bIt->bbMin[0]) * double(
                                       bIt->bbMax[1] - bIt->bbMin[1]));
        ...
        if(capturingTiePoint && currentBlob != 0) {
            ...
            if(numCaptureFrames == 0) {
                ...
                std::cout << " done" << std::endl;
                std::cout << "Tie point: " << tp.p[0] << ", " << tp.p[1] << "; " << tp.o[0] << ", " << tp.o[1] <<
                          ", " << tp.o[2] << std::endl;
...
void CalibrateProjector::display(GLContextData& contextData) const {
    ...
    if(capturingBackground) {
        ...
        if(dataItem->blobImageVersion != blobImageVersion) {
            ...
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, frameSize[0], frameSize[1], GL_RGB, GL_UNSIGNED_BYTE,
                            blobImage);
...
void CalibrateProjector::startBackgroundCapture(void) {
    ...
    std::cout << "CalibrateProjector: Capturing " << numBackgroundFrames << " background frames..." <<
              std::flush;
    camera->captureBackground(numBackgroundFrames, true, Misc::createFunctionCall(this,
...
void CalibrateProjector::calcCalibration(void) {
    ...
    for(std::vector<TiePoint>::iterator tpIt = tiePoints.begin(); tpIt != tiePoints.end(); ++tpIt) {
        std::cout << "Tie point: " << tpIt->p[0] << ", " << tpIt->p[1] << ", " << tpIt->o[0] << ", " <<
                  tpIt->o[1] << ", " << tpIt->o[2] << std::endl;
