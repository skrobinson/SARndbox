...
void CalibrateProjector::frame(void) {
    ...
    if(rawFrames.lockNewValue()) {
        ...
        rawBlobs = findBlobs(size, static_cast<unsigned short*>(rawFrames.getLockedValue().getBuffer()),
                             bp);
    }
...
void CalibrateProjector::startCapture(void) {
    ...
    std::cout << "CalibrateProjector: Capturing " << numCaptureFrames << " depth frame..." <<
              std::flush;
}
