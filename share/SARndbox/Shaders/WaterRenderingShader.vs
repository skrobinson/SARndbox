...
uniform mat4
tangentModelviewGridMatrix; // Tangend plane transformation from grid space to eye space
...
void accumulateLight0(in vec4 vertexEc, in vec3 normalEc, in vec4 ambient, in vec4 diffuse,
                      in vec4 specular, in float shininess, inout vec4 ambientDiffuseAccum, inout vec4 specularAccum) {
    ...
    if(sl >= gl_LightSource[0].spotCosCutoff) {
        ...
        float att = 1.0 / ((gl_LightSource[0].quadraticAttenuation * lightDist +
                            gl_LightSource[0].linearAttenuation) * lightDist + gl_LightSource[0].constantAttenuation);
...
void main() {
    ...
    vec4 tangentGc;
    tangentGc.x = texture2DRect(quantitySampler, vec2(vertexGc.x - 1.0,
                                vertexGc.y)).r - texture2DRect(quantitySampler, vec2(vertexGc.x + 1.0, vertexGc.y)).r;
    tangentGc.y = texture2DRect(quantitySampler, vec2(vertexGc.x,
                                vertexGc.y - 1.0)).r - texture2DRect(quantitySampler, vec2(vertexGc.x, vertexGc.y + 1.0)).r;
    tangentGc.z = 2.0;
    ...
    accumulateLight0(vertexEc, normalEc, gl_FrontMaterial.ambient, gl_FrontMaterial.diffuse,
                     gl_FrontMaterial.specular, gl_FrontMaterial.shininess, diffColor, specColor);
