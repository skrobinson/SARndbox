...
uniform mat4
tangentDepthProjection; // Transformation from depth image space to camera space for tangent planes
uniform vec4 basePlane; // Plane equation of the base plane
uniform vec2
heightColorMapTransformation; // Transformation from elevation to height color map texture coordinate
uniform mat4
waterLevelTextureTransformation; // Transformation from camera space to water level texture coordinate space
uniform mat4
shadowProjection; // Transformation from camera space to shadow texture coordinate space
...
void main() {
    ...
    heightColorMapTexCoord = elevation * heightColorMapTransformation.x +
                             heightColorMapTransformation.y;
    ...
    vec4 tangentDic;
    tangentDic.x = texture2DRect(depthSampler, vec2(vertexDic.x + 1.0,
                                 vertexDic.y)).r - texture2DRect(depthSampler, vec2(vertexDic.x - 1.0, vertexDic.y)).r;
    tangentDic.y = texture2DRect(depthSampler, vec2(vertexDic.x,
                                 vertexDic.y + 1.0)).r - texture2DRect(depthSampler, vec2(vertexDic.x, vertexDic.y - 1.0)).r;
    tangentDic.z = -2.0;
    ...
    vec3 lightDirEc = gl_LightSource[0].position.xyz * vertexEc.w - vertexEc.xyz *
    ...
    if(nl > 0.0) {
        ...
        specColor += (gl_LightSource[0].specular * gl_FrontMaterial.specular) * pow(nhv,
                     gl_FrontMaterial.shininess);
    }

    /* Attenuate the per-source light terms: */
    float att = 1.0 / ((gl_LightSource[0].quadraticAttenuation * lightDistEc +
                        gl_LightSource[0].linearAttenuation) * lightDistEc + gl_LightSource[0].constantAttenuation);
    diffColor *= att;
    specColor *= att;
