...
void main() {
    ...
    vec3 q0n = texture2DRect(quantitySampler, vec2(gl_FragCoord.x,
                             gl_FragCoord.y - 1.0)).rgb + texture2DRect(slopeYSampler, vec2(gl_FragCoord.x,
                                     gl_FragCoord.y - 1.0)).rgb * (cellSize.y * 0.5);
    vec3 q1e = texture2DRect(quantitySampler, vec2(gl_FragCoord.x - 1.0,
                             gl_FragCoord.y)).rgb + texture2DRect(slopeXSampler, vec2(gl_FragCoord.x - 1.0,
                                     gl_FragCoord.y)).rgb * (cellSize.x * 0.5);
    ...
    vec3 q2s = q2 - qy2;
    vec3 q2n = q2 + qy2;
    vec3 q3w = texture2DRect(quantitySampler, vec2(gl_FragCoord.x + 1.0,
                             gl_FragCoord.y)).rgb - texture2DRect(slopeXSampler, vec2(gl_FragCoord.x + 1.0,
                                     gl_FragCoord.y)).rgb * (cellSize.x * 0.5);
    vec3 q4s = texture2DRect(quantitySampler, vec2(gl_FragCoord.x,
                             gl_FragCoord.y + 1.0)).rgb - texture2DRect(slopeYSampler, vec2(gl_FragCoord.x,
                                     gl_FragCoord.y + 1.0)).rgb * (cellSize.y * 0.5);
