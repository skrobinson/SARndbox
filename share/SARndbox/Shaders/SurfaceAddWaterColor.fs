...
void addWaterColor(in vec2 fragCoord, inout vec4 baseColor) {
    ...
    float b = (texture2DRect(bathymetrySampler, vec2(waterLevelTexCoord.x - 1.0,
                             waterLevelTexCoord.y - 1.0)).r +
    ...
    if(waterLevel > 0.0) {
        ...
        vec3 wn = normalize(vec3(texture2DRect(quantitySampler, vec2(waterLevelTexCoord.x - 1.0,
                                               waterLevelTexCoord.y)).r -
...
void addWaterColorAdvected(inout vec4 baseColor) {
    ...
    if(waterLevelTex.b >= 1.0 / 2048.0) {
        ...
        float colorW = max(turb(vec3(waterLevelTex.rg * 0.05 / waterLevelTex.b,
                                     waterAnimationTime * 0.25)), 0.0); // Turbulence noise
