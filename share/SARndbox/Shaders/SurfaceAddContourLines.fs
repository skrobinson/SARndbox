...
void addContourLines(in vec2 fragCoord, inout vec4 baseColor) {
    ...
    *********************************************************************/
    ...
    float corner3 = texture2DRect(pixelCornerElevationSampler, vec2(fragCoord.x + 1.0,
                                  fragCoord.y + 1.0)).r;
    ...
    *********************************************************************/
    ...
    float corner0 = floor(texture2DRect(pixelCornerElevationSampler, vec2(fragCoord.x,
                                        fragCoord.y)).r * contourLineFactor);
    float corner1 = floor(texture2DRect(pixelCornerElevationSampler, vec2(fragCoord.x + 1.0,
                                        fragCoord.y)).r * contourLineFactor);
    float corner2 = floor(texture2DRect(pixelCornerElevationSampler, vec2(fragCoord.x,
                                        fragCoord.y + 1.0)).r * contourLineFactor);
    float corner3 = floor(texture2DRect(pixelCornerElevationSampler, vec2(fragCoord.x + 1.0,
                                        fragCoord.y + 1.0)).r * contourLineFactor);
    ...
    if(numEdges > 2 || edgeMask == 3 || edgeMask == 12 || (numEdges == 2
            && mod(floor(fragCoord.x) + floor(fragCoord.y), 2.0) == 0.0)) {
        /* Topographic contour lines are rendered in black: */
        baseColor = vec4(0.0, 0.0, 0.0, 1.0);
    }
