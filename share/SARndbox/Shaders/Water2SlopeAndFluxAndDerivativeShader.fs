...
void main() {
    ...
    float b0 = (texture2DRect(bathymetrySampler, vec2(gl_FragCoord.x - 1.0,
                              gl_FragCoord.y - 2.0)).r + texture2DRect(bathymetrySampler, vec2(gl_FragCoord.x,
                                      gl_FragCoord.y - 2.0)).r) * 0.5;
    float b1 = (b00 + b10) * 0.5;
    float b2 = (texture2DRect(bathymetrySampler, vec2(gl_FragCoord.x - 2.0,
                              gl_FragCoord.y - 1.0)).r + texture2DRect(bathymetrySampler, vec2(gl_FragCoord.x - 2.0,
                                      gl_FragCoord.y)).r) * 0.5;
    float b3 = (b00 + b01) * 0.5;
    float b4 = (b10 + b11) * 0.5;
    float b5 = (texture2DRect(bathymetrySampler, vec2(gl_FragCoord.x + 1.0,
                              gl_FragCoord.y - 1.0)).r + texture2DRect(bathymetrySampler, vec2(gl_FragCoord.x + 1.0,
                                      gl_FragCoord.y)).r) * 0.5;
    float b6 = (b01 + b11) * 0.5;
    float b7 = (texture2DRect(bathymetrySampler, vec2(gl_FragCoord.x - 1.0,
                              gl_FragCoord.y + 1.0)).r + texture2DRect(bathymetrySampler, vec2(gl_FragCoord.x,
                                      gl_FragCoord.y + 1.0)).r) * 0.5;
    ...
    vec3 q1n = q1 + calcSlope(texture2DRect(quantitySampler, vec2(gl_FragCoord.x,
                                            gl_FragCoord.y - 2.0)).rgb, q1, q4, cellSize.y, b0, b1) * (cellSize.y * 0.5);
    vec3 q3e = q3 + calcSlope(texture2DRect(quantitySampler, vec2(gl_FragCoord.x - 2.0,
                                            gl_FragCoord.y)).rgb, q3, q4, cellSize.x, b2, b3) * (cellSize.x * 0.5);
    ...
    vec3 q4s = q4 - q4y;
    vec3 q4n = q4 + q4y;
    vec3 q5w = q5 - calcSlope(q4, q5, texture2DRect(quantitySampler, vec2(gl_FragCoord.x + 2.0,
                              gl_FragCoord.y)).rgb, cellSize.x, b4, b5) * (cellSize.x * 0.5);
    vec3 q7s = q7 - calcSlope(q4, q7, texture2DRect(quantitySampler, vec2(gl_FragCoord.x,
                              gl_FragCoord.y + 2.0)).rgb, cellSize.y, b6, b7) * (cellSize.y * 0.5);
    ...
    gl_FragData[0] = vec4(source - (fluxXe - fluxXw) / cellSize.x - (fluxYn - fluxYs) / cellSize.y,
                          0.0);
}
