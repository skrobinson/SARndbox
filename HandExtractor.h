...
class HandExtractor {
  ...
  public:
    ...
    HandsExtractedFunction; // Type for functions called when a new hand list has been extracted
  ...
  private:
    ...
    unsigned int
    maxDepthDist; // Maximum depth distance between adjacent pixels to belong to the same foreground blob
    unsigned int minBlobSize,
             maxBlobSize; // Minimum and maximum number of pixels to consider a blob a hand candidate
    ...
    handsExtractedFunction; // Function called when a new list of extracted hands is ready
  ...
  public:
    HandExtractor(const unsigned int sDepthFrameSize[2],
                  ...
                  const PTransform& sDepthProjection); // Creates a hand extractor for depth frames of the given size
  private:
  ...
  public:
    ...
    unsigned int getMaxDepthDist(void)
    const { // Returns the maximum depth distance between adjacent pixels to belong to the same foreground blob
        return maxDepthDist;
    }
    void setMaxDepthDist(unsigned int
                         newMaxDepthDist); // Sets the maximum depth distance between adjacent pixels to belong to the same foreground blob
    unsigned int getMinBlobSize(void)
    const { // Returns the minimum number of pixels to consider a blob a hand candidate
        return minBlobSize;
    }
    unsigned int getMaxBlobSize(void)
    const { // Returns the maximum number of pixels to consider a blob a hand candidate
        return minBlobSize;
    }
    void setBlobSizeRange(unsigned int newMinBlobSize,
                          unsigned int newMaxBlobSize); // Sets the range of numbers of pixels to consider a blob a hand candidate
    ...
    int getMaxCornerEnterDist(void)
    const { // Returns the maximum distance between snake's head and tail to enter corner state
        return maxCornerEnterDist;
    }
    int getMinCenterDist(void)
    const { // Returns the minimum distance from snake's center to line defined by its head and tail to enter corner state
        return minCenterDist;
    }
    int getMinCornerExitDist(void)
    const { // Returns the minimum distance between snake's head and tail to exit corner state
        return minCornerExitDist;
    }
    void setCornerDists(int newMaxCornerEnterDist, int newMinCenterDist,
                        int newMinCornerExitDist); // Sets distances between snake's head and tail to enter and exit corner state, respectively
    ...
    void setHandsExtractedFunction(HandsExtractedFunction*
                                   newHandsExtractedFunction); // Sets the output function; adopts given functor object
    void receiveRawFrame(const Kinect::FrameBuffer&
                         newFrame); // Called to receive a new raw depth frame
    bool lockNewExtractedHands(
        void) { // Locks the most recently produced output list of extracted hands for reading; returns true if the locked list is new
