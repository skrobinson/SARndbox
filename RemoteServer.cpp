...
void RemoteServer::frame(double applicationTime) {
    ...
    if(numClients > 0 && applicationTime >= nextRequestTime) {
        ...
        if(sandbox->gridRequest.requestGrids(gb.bathymetry, gb.waterLevel, &RemoteServer::readBackCallback,
                                             this)) {
...
void RemoteServer::glRenderAction(GLContextData& contextData) const {
    ...
    if(!positions.empty()) {
        ...
        for(std::vector<Vrui::ONTransform>::const_iterator pIt = positions.begin(); pIt != positions.end();
                ++pIt) {
            glPushMatrix();
