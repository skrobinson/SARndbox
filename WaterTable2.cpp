...
WaterTable2::DataItem::DataItem(void)
    : currentBathymetry(0), bathymetryVersion(0), currentQuantity(0),
      ...
      bathymetryFramebufferObject(0), derivativeFramebufferObject(0), maxStepSizeFramebufferObject(0),
      integrationFramebufferObject(0), waterFramebufferObject(0),
      bathymetryShader(0), waterAdaptShader(0), derivativeShader(0), maxStepSizeShader(0),
      boundaryShader(0), eulerStepShader(0), rungeKuttaStepShader(0), waterAddShader(0), waterShader(0) {
    for(int i = 0; i < 2; ++i) {
...
GLfloat WaterTable2::calcDerivative(WaterTable2::DataItem* dataItem, GLuint quantityTextureObject,
    ...
    if(calcMaxStepSize) {
        ...
        while(reducedWidth > 1 || reducedHeight > 1) {
            ...
            glUniformARB(dataItem->maxStepSizeShaderUniformLocations[0], GLfloat(reducedWidth - 1),
                         GLfloat(reducedHeight - 1));
...
void WaterTable2::initContext(GLContextData& contextData) const {
...

    {
        ...
        for(int i = 0; i < 2; ++i) {
            ...
            glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_R32F, size[0] - 1, size[1] - 1, 0, GL_LUMINANCE,
                         GL_FLOAT, b);
        }
        delete[] b;
    }
...

    {
        ...
        for(int i = 0; i < 2; ++i) {
            ...
            glTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_WRAP_S, GL_CLAMP);
            glTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_WRAP_T, GL_CLAMP);
            glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_R32F, size[0], size[1], 0, GL_LUMINANCE, GL_FLOAT,
                         mss);
        }
        delete[] mss;
    }
...

    {
        ...
        for(int i = 0; i < 2; ++i)
            glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT + i,
                                      GL_TEXTURE_RECTANGLE_ARB, dataItem->bathymetryTextureObjects[i], 0);
...

    {
        ...
        for(int i = 0; i < 2; ++i)
            glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT + i,
                                      GL_TEXTURE_RECTANGLE_ARB, dataItem->maxStepSizeTextureObjects[i], 0);
        glDrawBuffer(GL_NONE);
        glReadBuffer(GL_NONE);
    }

    {
        ...
        for(int i = 0; i < 3; ++i)
            glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT + i,
                                      GL_TEXTURE_RECTANGLE_ARB, dataItem->quantityTextureObjects[i], 0);
        glDrawBuffer(GL_NONE);
        glReadBuffer(GL_NONE);
    }
    ...
    static const char* vertexShaderSourceTemplate =
        "void main(){gl_Position=vec4(gl_Vertex.x*%f-1.0,gl_Vertex.y*%f-1.0,0.0,1.0);}";
    char vertexShaderSource[256];
    snprintf(vertexShaderSource, sizeof(vertexShaderSource), vertexShaderSourceTemplate,
             2.0 / double(size[0]), 2.0 / double(size[1]));
    ...
    /* Create the bathymetry update shader: */
    {
        ...
        dataItem->bathymetryShaderUniformLocations[0] = glGetUniformLocationARB(dataItem->bathymetryShader,
                "oldBathymetrySampler");
        dataItem->bathymetryShaderUniformLocations[1] = glGetUniformLocationARB(dataItem->bathymetryShader,
                "newBathymetrySampler");
        dataItem->bathymetryShaderUniformLocations[2] = glGetUniformLocationARB(dataItem->bathymetryShader,
                "quantitySampler");
    }
    ...
    /* Create the water adaptation shader: */
    {
        ...
        glDeleteObjectARB(vertexShader);
        glDeleteObjectARB(fragmentShader);
        dataItem->waterAdaptShaderUniformLocations[0] = glGetUniformLocationARB(dataItem->waterAdaptShader,
                "bathymetrySampler");
        dataItem->waterAdaptShaderUniformLocations[1] = glGetUniformLocationARB(dataItem->waterAdaptShader,
                "newQuantitySampler");
    }
    ...
    /* Create the temporal derivative computation shader: */
    {
        ...
        glDeleteObjectARB(vertexShader);
        glDeleteObjectARB(fragmentShader);
        dataItem->derivativeShaderUniformLocations[0] = glGetUniformLocationARB(dataItem->derivativeShader,
                "cellSize");
        dataItem->derivativeShaderUniformLocations[1] = glGetUniformLocationARB(dataItem->derivativeShader,
                "theta");
        dataItem->derivativeShaderUniformLocations[2] = glGetUniformLocationARB(dataItem->derivativeShader,
                "g");
        dataItem->derivativeShaderUniformLocations[3] = glGetUniformLocationARB(dataItem->derivativeShader,
                "epsilon");
        dataItem->derivativeShaderUniformLocations[4] = glGetUniformLocationARB(dataItem->derivativeShader,
                "bathymetrySampler");
        dataItem->derivativeShaderUniformLocations[5] = glGetUniformLocationARB(dataItem->derivativeShader,
                "quantitySampler");
    }
    ...
    /* Create the boundary condition shader: */
    {
        ...
        glDeleteObjectARB(vertexShader);
        glDeleteObjectARB(fragmentShader);
        dataItem->boundaryShaderUniformLocations[0] = glGetUniformLocationARB(dataItem->boundaryShader,
                "bathymetrySampler");
    }
    ...
    /* Create the Euler integration step shader: */
    {
        ...
        glDeleteObjectARB(vertexShader);
        glDeleteObjectARB(fragmentShader);
        dataItem->eulerStepShaderUniformLocations[0] = glGetUniformLocationARB(dataItem->eulerStepShader,
                "stepSize");
        dataItem->eulerStepShaderUniformLocations[1] = glGetUniformLocationARB(dataItem->eulerStepShader,
                "attenuation");
        dataItem->eulerStepShaderUniformLocations[2] = glGetUniformLocationARB(dataItem->eulerStepShader,
                "quantitySampler");
        dataItem->eulerStepShaderUniformLocations[3] = glGetUniformLocationARB(dataItem->eulerStepShader,
                "derivativeSampler");
    }
    ...
    /* Create the water adder rendering shader: */
    {
        ...
        glDeleteObjectARB(vertexShader);
        glDeleteObjectARB(fragmentShader);
        dataItem->waterAddShaderUniformLocations[0] = glGetUniformLocationARB(dataItem->waterAddShader,
                "pmv");
        dataItem->waterAddShaderUniformLocations[1] = glGetUniformLocationARB(dataItem->waterAddShader,
                "stepSize");
        dataItem->waterAddShaderUniformLocations[2] = glGetUniformLocationARB(dataItem->waterAddShader,
                "waterSampler");
    }
    ...
    /* Create the water shader: */
    {
        ...
        glDeleteObjectARB(vertexShader);
        glDeleteObjectARB(fragmentShader);
        dataItem->waterShaderUniformLocations[0] = glGetUniformLocationARB(dataItem->waterShader,
                "bathymetrySampler");
        dataItem->waterShaderUniformLocations[1] = glGetUniformLocationARB(dataItem->waterShader,
                "quantitySampler");
        dataItem->waterShaderUniformLocations[2] = glGetUniformLocationARB(dataItem->waterShader,
                "waterSampler");
    }
}
...
void WaterTable2::removeRenderFunction(const AddWaterFunction* removeRenderFunction) {
    ...
    for(std::vector<const AddWaterFunction*>::iterator rfIt = renderFunctions.begin();
...
void WaterTable2::updateBathymetry(const GLfloat* bathymetryGrid,
    ...
    glBindTexture(GL_TEXTURE_RECTANGLE_ARB,
                  dataItem->bathymetryTextureObjects[1 - dataItem->currentBathymetry]);
    glUniform1iARB(dataItem->bathymetryShaderUniformLocations[1], 1);
    glTexSubImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, size[0] - 1, size[1] - 1, GL_LUMINANCE,
                    GL_FLOAT, bathymetryGrid);
...
GLfloat WaterTable2::runSimulationStep(bool forceStepSize, GLContextData& contextData) const {
    ...
    GLfloat stepSize = calcDerivative(dataItem,
                                      dataItem->quantityTextureObjects[dataItem->currentQuantity], !forceStepSize);
    ...
    if(waterDeposit != 0.0f || !renderFunctions.empty()) {
        ...
        for(std::vector<const AddWaterFunction*>::const_iterator rfIt = renderFunctions.begin();
                rfIt != renderFunctions.end(); ++rfIt)
            (**rfIt)(contextData);
