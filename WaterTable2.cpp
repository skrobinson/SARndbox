...
WaterTable2::DataItem::DataItem(void)
    : quantityTextureObject(0), derivativeTextureObject(0), quantityStarTextureObject(0),
      ...
      bathymetryShader(0), derivativeShader(0), maxStepSizeShader(0), boundaryShader(0),
      eulerStepShader(0), rungeKuttaStepShader(0), waterAddShader(0), waterShader(0) {
    for(int i = 0; i < 2; ++i) {
...
WaterTable2::WaterTable2(GLsizei width, GLsizei height, const Plane& basePlane,
...
void WaterTable2::initContext(GLContextData& contextData) const {
...

    {
        ...
        glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_RECTANGLE_ARB,
                                  dataItem->quantityTextureObject, 0);
        glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT1_EXT, GL_TEXTURE_RECTANGLE_ARB,
                                  dataItem->quantityStarTextureObject, 0);
        glDrawBuffer(GL_NONE);
        glReadBuffer(GL_NONE);
    }
        ...
        for(int x = 0; x < size[0] - 1; ++x, ++bPtr) {
            ...
            *bPtr = domain.min[2];
            ...
            if(x > 0 && x < size[0] - 2 && y > 0 && y < size[1] - 2) {
                ...
                GLfloat arg = Math::exp(-(Math::sqr(GLfloat(x) - cx) + Math::sqr(GLfloat(y) - cy)) / Math::sqr(
                                            20.0f)) * 15.0f;
                *bPtr = arg;
            } else
                *bPtr = 25.0f;
...
void WaterTable2::updateBathymetry(const SurfaceRenderer& bathymetryRenderer,
    ...
    glOrtho(domain.min[0] + hw, domain.max[0] - hw, domain.min[1] + hh, domain.max[1] - hh,
...
GLfloat WaterTable2::runSimulationStep(GLContextData& contextData) const {
    ...
    glPushMatrix();
    glLoadIdentity();
    glOrtho(0.0, double(size[0]), 0.0, double(size[1]), -1.0,
            1.0); // Set projection matrix for pixel-coordinate rendering
    glMatrixMode(GL_MODELVIEW);
    glPushMatrix();
    glLoadIdentity();
    ...
    if(waterDeposit != 0.0f || !renderFunctions.empty()) {
        ...
        glMatrixMode(GL_PROJECTION);
        glLoadIdentity();
        glOrtho(domain.min[0], domain.max[0], domain.min[1], domain.max[1], -domain.max[2],
                -domain.min[2]);
        glMatrixMode(GL_MODELVIEW);
        glLoadMatrix(baseTransform);
