  ...
  private:
    ...
    struct DataItem: public GLObject::DataItem { // Structure storing per-context OpenGL state
      ...
      public:
        ...
        GLhandleARB
        elevationShader; // Shader program to render the surface's elevation relative to a plane
    ...
    /* Constructors and destructors: */
  public:
    DepthImageRenderer(const unsigned int
                       sDepthImageSize[2]); // Creates an elevation renderer for the given depth image size
    ...
    void setIntrinsics(const Kinect::FrameSource::IntrinsicParameters&
                       ips); // Sets a new depth unprojection matrix and, if present, 2D lens distortion parameters
    ...
    Scalar intersectLine(const Point& p0, const Point& p1, Scalar elevationMin,
                         Scalar elevationMax)
    const; // Intersects a line segment with the current depth image in camera space; returns intersection point's parameter along line
    unsigned int getDepthImageVersion(void)
    const { // Returns the version number of the current depth image
        return depthImageVersion;
    }
    void uploadDepthProjection(GLint location)
    const; // Uploads the depth unprojection matrix into the GLSL 4x4 matrix at the given uniform location
    void bindDepthTexture(GLContextData& contextData)
    const; // Binds the up-to-date depth texture image to the currently active texture unit
    ...
    void renderElevation(const PTransform& projectionModelview,
                         GLContextData& contextData) const; // Renders the surface's elevation relative to the base plane into the current one-component floating-point valued frame buffer
};
