...
class RainMaker {
  ...
  public:
    ...
    OutputBlobsFunction; // Type for functions called when a new object list has been extracted
  ...
  private:
    ...
    OutputBlobsFunction*
    outputBlobsFunction; // Function called when a new (potentially empty) object list has been extracted

    /* Private methods: */
    template <class DepthPixelParam>
    void extractBlobs(const Kinect::FrameBuffer& depthFrame, const ValidPixelProperty& vpp,
                      BlobList& blobsCc);
    void* detectionThreadMethod(void); // Method for the object detection thread

    /* Constructors and destructors: */
  public:
    RainMaker(const int sDepthSize[2], const int sColorSize[2], const PTransform& sDepthProjection,
              const PTransform& sColorProjection, const Plane& basePlane, double minElevation,
              double maxElevation, int
              sMinBlobSize); // Creates an object detector for frames of the given size and the given range of elevation values relative to the given base plane in camera space
    ~RainMaker(void); // Destroys the object detector

    /* Methods: */
    void setDepthIsFloat(bool
                         newDepthIsFloat); // Sets whether incoming depth frames have float pixel values
    void setOutputBlobsFunction(OutputBlobsFunction*
                                newOutputBlobsFunction); // Sets the output function; adopts given functor object
