...
SurfaceRenderer::SurfaceRenderer(const unsigned int sSize[2],
    ...
    fileMonitor.addPath((std::string(SHADERDIR) + std::string("/SurfaceAddContourLines.fs")).c_str(),
                        IO::FileMonitor::Modified, Misc::createFunctionCall(this,
                                &SurfaceRenderer::shaderSourceFileChanged));
    fileMonitor.addPath((std::string(SHADERDIR) + std::string("/SurfaceIlluminate.fs")).c_str(),
                        IO::FileMonitor::Modified, Misc::createFunctionCall(this,
                                &SurfaceRenderer::shaderSourceFileChanged));
    fileMonitor.addPath((std::string(SHADERDIR) + std::string("/SurfaceAddWaterColor.fs")).c_str(),
                        IO::FileMonitor::Modified, Misc::createFunctionCall(this,
                                &SurfaceRenderer::shaderSourceFileChanged));
...
void SurfaceRenderer::initContext(GLContextData& contextData) const {
    ...
    glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB, (size[1] - 1)*size[0] * 2 * sizeof(GLuint), 0,
                    GL_STATIC_DRAW_ARB);
    ...
    glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_LUMINANCE32F_ARB, size[0], size[1], 0, GL_LUMINANCE,
                 GL_UNSIGNED_BYTE, 0);
...
void SurfaceRenderer::setHeightMapRange(GLsizei newHeightMapSize, GLfloat newMinElevation,
                                        GLfloat newMaxElevation) {
    /* Calculate the new height map elevation scaling and offset coefficients: */
    GLdouble hms = GLdouble(newHeightMapSize - 1) / ((newMaxElevation - newMinElevation) * GLdouble(
                       newHeightMapSize));
...
void SurfaceRenderer::glRenderDepthOnly(const SurfaceRenderer::PTransform& modelviewProjection,
...
void SurfaceRenderer::glPrepareContourLines(GLContextData& contextData) const {
    ...
    if(dataItem->contourLineFramebufferSize[0] != viewport[2] + 1
            || dataItem->contourLineFramebufferSize[1] != viewport[3] + 1) {
...
void SurfaceRenderer::glRenderSinglePass(GLuint heightColorMapTexture,
        GLContextData& contextData) const {
    /* Get the data item: */
    DataItem* dataItem = contextData.retrieveDataItem<DataItem>(this);
...
void SurfaceRenderer::glRenderGlobalAmbientHeightMap(GLuint heightColorMapTexture,
        GLContextData& contextData) const {
    /* Get the data item: */
    DataItem* dataItem = contextData.retrieveDataItem<DataItem>(this);
...
void SurfaceRenderer::glRenderShadowedIlluminatedHeightMap(GLuint heightColorMapTexture,
        GLuint shadowTexture, const SurfaceRenderer::PTransform& shadowProjection,
        GLContextData& contextData) const {
    /* Get the data item: */
    DataItem* dataItem = contextData.retrieveDataItem<DataItem>(this);
