...
class FrameFilter {
  ...
  public:
    ...
    /* Elements: */
  private:
    int size[2]; // Width and height of processed frames
    bool hasDepthCorrection; // Flag whether the camera has per-pixel depth correction
  ...
  public:
    FrameFilter(const int sSize[2], int sNumAveragingSlots, const PTransform& depthProjection,
                const Plane&
    ...
    /* Methods: */
    void setDepthCorrection(const Kinect::FrameBuffer&
                            newDepthCorrection); // Enables per-pixel depth correction by passing a buffer of depth correction coefficients
    ...
    void setOutputFrameFunction(OutputFrameFunction*
