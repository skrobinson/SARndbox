...
Sandbox::WaterTool::WaterTool(const Vrui::ToolFactory* factory,
...
void Sandbox::WaterTool::buttonCallback(int buttonSlotIndex,
...
Sandbox::LocalWaterToolFactory* Sandbox::LocalWaterTool::initClass(Vrui::ToolManager&
        toolManager) {
    /* Create the tool factory: */
...
Sandbox::LocalWaterTool::LocalWaterTool(const Vrui::ToolFactory* factory,
                                        const Vrui::ToolInputAssignment& inputAssignment)
    : Vrui::Tool(factory, inputAssignment),
      addWaterFunction(0),
      adding(0.0f) {
...
void Sandbox::LocalWaterTool::buttonCallback(int buttonSlotIndex,
        Vrui::InputDevice::ButtonCallbackData* cbData) {
    GLfloat waterAmount = application->rainStrength;
    if(!cbData->newButtonState)
        waterAmount = -waterAmount;
...
void Sandbox::LocalWaterTool::glRenderActionTransparent(GLContextData& contextData) const {
    ...
    Vrui::Point rainPos = Vrui::getInverseNavigationTransformation().transform(getButtonDevicePosition(
                              0));
    Vrui::Scalar rainRadius = Vrui::getPointPickDistance() * Vrui::Scalar(3);

    /* Construct the rain cylinder: */
    Vrui::Vector z = application->waterTable->getBaseTransform().inverseTransform(Vrui::Vector(0, 0,
                     1));
    ...
    for(int i = 0; i <= 32; ++i) {
        Vrui::Scalar angle = Vrui::Scalar(2) * Math::Constants<Vrui::Scalar>::pi * Vrui::Scalar(
                                 i) / Vrui::Scalar(32);
        glNormal(x * Math::cos(angle) + y * Math::sin(angle));
        glVertex(rainPos + x * (Math::cos(angle)*rainRadius) + y * (Math::sin(angle)*rainRadius));
        glVertex(rainPos + x * (Math::cos(angle)*rainRadius) + y * (Math::sin(
                     angle)*rainRadius) - z * Vrui::Scalar(50));
    }
    glEnd();
...
void Sandbox::addWater(GLContextData& contextData) const {
    ...
    if(!rainObjects.getLockedValue().empty()) {
        ...
        for(RainMaker::BlobList::const_iterator roIt = rainObjects.getLockedValue().begin();
                roIt != rainObjects.getLockedValue().end(); ++roIt) {
            /* Render the rain object: */
            glBegin(GL_POLYGON);
            for(int i = 0; i < 32; ++i) {
                Vrui::Scalar angle = Vrui::Scalar(2) * Math::Constants<Vrui::Scalar>::pi * Vrui::Scalar(
                                         i) / Vrui::Scalar(32);
                glVertex(roIt->centroid + x * (Math::cos(angle)*roIt->radius) + y * (Math::sin(
                             angle)*roIt->radius));
            }
            glEnd();
        }
...
GLMotif::PopupMenu* Sandbox::createMainMenu(void) {
    ...
    GLMotif::ToggleButton* pauseUpdatesButton = new GLMotif::ToggleButton("PauseUpdatesButton",
            mainMenu, "Pause Topography");
...
Sandbox::Sandbox(int& argc, char**& argv, char**& appDefaults)
    ...
    Misc::SelfDestructPointer<Kinect::FrameSource::DepthCorrection> depthCorrection(
        camera->getDepthCorrectionParameters());
    ...
    Geometry::Point<double, 3> basePlaneCorners[4];
    {
        ...
        for(int i = 0; i < 4; ++i) {
            ...
            basePlaneCorners[i] = Misc::ValueCoder<Geometry::Point<double, 3> >::decode(s.c_str(),
                                  s.c_str() + s.length());
        }
    }
    ...
    if(Misc::hasCaseExtension(heightColorMapFileName.c_str(), ".cpt")) {
        ...
        while(!heightMapSource.eof()) {
            ...
            if(!heightMapSource.isLiteral('\n'))
                Misc::throwStdErr("Sandbox::Sandbox: Format error in color map file %s",
                                  heightColorMapFileName.c_str());
        }
    } else {
        heightMapSource.setPunctuation(",\n");
        heightMapSource.skipWs();
    ...
    frameFilter->setValidElevationInterval(cameraIps.depthProjection, basePlane, elevationMin,
                                           elevationMax);
    ...
    rainMaker = new RainMaker(frameSize, camera->getActualFrameSize(Kinect::FrameSource::COLOR),
                              cameraIps.depthProjection, cameraIps.colorProjection, basePlane, rainElevationMin,
                              rainElevationMax, 20);
    ...
    rmFrameFilter->setValidElevationInterval(cameraIps.depthProjection, basePlane, rainElevationMin,
            rainElevationMax);
    ...
    rmFrameFilter->setOutputFrameFunction(Misc::createFunctionCall(rainMaker,
                                          &RainMaker::receiveRawDepthFrame));

    /* Start streaming depth frames: */
    camera->startStreaming(Misc::createFunctionCall(rainMaker, &RainMaker::receiveRawColorFrame),
    ...
    surfaceRenderer->setHeightMapRange(heightMap.getNumEntries(), heightMap.getScalarRangeMin(),
                                       heightMap.getScalarRangeMax());
    ...
    if(waterTable != 0 && waterSpeed > 0.0) {
        if(renderWaterSurface) {
            ...
            waterTransform.getMatrix()(0, 0) = SurfaceRenderer::Scalar(wd.max[0] - wd.min[0]) /
                                               SurfaceRenderer::Scalar(wtSize[0]);
            waterTransform.getMatrix()(0, 3) = SurfaceRenderer::Scalar(wd.min[0]);
            waterTransform.getMatrix()(1, 1) = SurfaceRenderer::Scalar(wd.max[1] - wd.min[1]) /
                                               SurfaceRenderer::Scalar(wtSize[1]);
    ...
    Vrui::setNavigationTransformation(c, maxDist,
                                      Geometry::normal(Vrui::Vector(basePlane.getNormal())));
}
        ...
        if(useShadows) {
            ...
            for(int lightSourceIndex = 0; lightSourceIndex < lt.getMaxNumLights(); ++lightSourceIndex)
                if(lt.getLightState(lightSourceIndex).isEnabled()) {
                    ...
                    surfaceRenderer->glRenderShadowedIlluminatedHeightMap(dataItem->heightColorMapObject,
...
void Sandbox::initContext(GLContextData& contextData) const {
    ...
    glTexImage1D(GL_TEXTURE_1D, 0, GL_RGB8, heightMap.getNumEntries(), 0, GL_RGBA, GL_FLOAT,
                 heightMap.getColors());
